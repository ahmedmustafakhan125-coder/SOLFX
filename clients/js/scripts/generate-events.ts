...
/**
 * Which program. `solfx_core` by default, from the IDL `anchor build` writes; `noxfunds` from the
 * IDL **as published on chain** (`clients/js/idl/noxfunds.json`), the same source its Codama
 * client is generated from, so the events decode what the deployed program actually emits.
 */
const TARGETS = {
  solfx_core: {
    idl: path.join("target", "idl", "solfx_core.json"),
    out: path.join(here, "..", "src", "events", "generated.ts"),
    registry: "SOLFX_EVENTS",
    nameType: "SolfxEventName",
  },
  noxfunds: {
    idl: path.join("clients", "js", "idl", "noxfunds.json"),
    out: path.join(here, "..", "src", "nox", "events.generated.ts"),
    registry: "NOX_EVENTS",
    nameType: "NoxEventName",
  },
} as const;
const which = (process.argv[2] ?? "solfx_core") as keyof typeof TARGETS;
const target = TARGETS[which];
if (!target) throw new Error(`unknown program ${which}; expected ${Object.keys(TARGETS).join(" or ")}`);
const idlPath = path.join(root, target.idl);
const outPath = target.out;
...
const imports = new Set<string>(["type Decoder", "getStructDecoder"]);
const blocks: string[] = [];
...
const out = `/**
 * GENERATED by scripts/generate-events.ts from ${target.idl}. DO NOT EDIT.
 *
...
export const ${target.registry} = [
${registry.join("\n")}
] as const;

export type ${target.nameType} = (typeof ${target.registry})[number]["name"];
`;
