...
async function main() {
  ...
  for (const [sym, id] of [
    ...
    console.log(
      `  ${sym.padEnd(8)} ${bars.length} bars in ${Date.now() - t0}ms  ${up} up / ${bars.length - up} down  ${withWick} with range`
    );
    const b = bars[bars.length - 1];
    if (b)
      console.log(
        `           last O ${b.open.toFixed(4)}  H ${b.high.toFixed(4)}  L ${b.low.toFixed(4)}  C ${b.close.toFixed(4)}`
      );
  }
}
void main();
