...
async function main() {
  ...
  for (const [sym, id] of [
    ...
    if (bars.length === 0) { console.log(`  ${sym}  no bars`); continue; }
    const lo = Math.min(...bars.map(b => b.value)), hi = Math.max(...bars.map(b => b.value));
    console.log(`  ${sym.padEnd(8)} ${bars.length} bars in ${ms}ms  range ${lo.toFixed(5)} – ${hi.toFixed(5)}`);
  }
}
void main();
