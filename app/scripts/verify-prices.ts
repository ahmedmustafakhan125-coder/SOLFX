...
async function main() {
  ...
  for (const sym of ["BTC/USD", "EUR/USD", "XAU/USD", "USD/JPY"]) {
    ...
    if (!acct) { console.log(`  ${sym.padEnd(8)} no account`); continue; }
    const { value } = await rpc.getAccountInfo(acct, { encoding: "base64" }).send();
    if (!value) { console.log(`  ${sym.padEnd(8)} account missing`); continue; }
    ...
    const frac = (price % PRICE_PRECISION).toString().padStart(9, "0").slice(0, 5);
    console.log(
