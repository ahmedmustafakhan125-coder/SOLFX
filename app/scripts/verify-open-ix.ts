...
import { buildOpenPosition, firstFreeNonce, priceLimitFor, OPEN_POSITION_CU } from "../src/lib/trade.js";
...
async function main() {
  ...
  console.log(`  oracle       ${price.price}  age ${price.ageSeconds}s${price.stale ? "  STALE" : ""}`);
  console.log(`  price limit  ${priceLimitFor(Direction.Long, price.price, slippageBps)}  (${slippageBps} bps)`);
  console.log(`  nonce        ${nonce}`);

  const ix = await buildOpenPosition({
    signer, marketIndex: market.index, direction: Direction.Long,
    sizeBase, collateral, price: price.price, slippageBps, priceUpdate: priceAccount, nonce,
  });
  ...
  console.log(`\nerr    : ${j(sim.value.err)}`);
  console.log(`units  : ${sim.value.unitsConsumed} (requested ${OPEN_POSITION_CU})`);
  ...
  if (events.length) {
    ...
    for (const e of events) {
      console.log(`  ${e.name}`);
      for (const [k, v] of Object.entries(e.data)) console.log(`    ${k.padEnd(20)} ${v}`);
    }
  }
}
void main();
