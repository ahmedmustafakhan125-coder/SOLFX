...
import {
  appendTransactionMessageInstructions, compileTransaction, createNoopSigner, createSolanaRpc,
...
import { buildClosePosition, closePriceLimit, loadPositions } from "../src/lib/positions.js";
...
async function main() {
  ...
  const map = new PriceAccountMap(JSON.parse(readFileSync("public/price-accounts.json", "utf8")));
  ...
  const positions = await loadPositions(rpc, owner, markets.map((m) => m.index), prices);
  console.log(`open positions: ${positions.length}`);
  for (const p of positions) {
    const m = markets.find((x) => x.index === p.marketIndex);
    console.log(`  ${m?.symbol} nonce ${p.nonce}  ${Direction[p.data.direction]}`);
    console.log(`    size        ${p.data.sizeBase}`);
    console.log(`    entry       ${p.data.entryPrice}`);
    console.log(`    collateral  ${usd(p.data.collateral)}`);
    console.log(`    notional@   ${usd(p.notionalNow)}   entry ${usd(p.data.entryNotional)}`);
  ...
  console.log(`  limit  ${closePriceLimit(pos.data.direction, price, 100)}  (100 bps, ${Direction[pos.data.direction] === "Long" ? "minimum — a sell" : "maximum — a buy"})`);

  const ix = await buildClosePosition({
    signer, marketIndex: pos.marketIndex, nonce: pos.nonce,
    direction: pos.data.direction, price, slippageBps: 100, priceUpdate,
  });
  ...
  for (const e of parseEvents(sim.value.logs ?? [])) {
    console.log(`\n  ${e.name}`);
    for (const [k, v] of Object.entries(e.data)) console.log(`    ${k.padEnd(22)} ${v}`);
  }
}
void main();
