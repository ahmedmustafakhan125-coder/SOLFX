...
import { fmtBps, fmtPrice, fmtRateBps, fmtUsd, priceplaces } from "@/lib/format";
...
export function MarketPanel({ market, price }: { market: LoadedMarket; price?: LivePrice }) {
    ...
    <div className="border-b border-line-soft px-5 py-4">
      <div className="flex flex-wrap items-baseline gap-x-8 gap-y-3">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-xl font-bold tracking-tight">{market.symbol}</h1>
        ...
        <div>
          <div className="text-[10px] uppercase tracking-[0.14em] text-ink-dim">Oracle</div>
        ...
        <Stat
          label="Age"
          value={price ? `${price.ageSeconds}s / ${MAX_STALENESS_SECONDS}s` : "—"}
      ...
      {!price ? (
        ...
        <p className="mt-3 rounded border border-warn/30 bg-warn/10 px-3 py-2 text-xs text-warn">
          This price is {price.ageSeconds}s old. The program refuses any trade against an
          oracle older than {MAX_STALENESS_SECONDS}s, so an order here would be rejected with
          <span className="tnum"> OracleStale</span>. Run the price poster against this
          cluster.
        </p>
      ) : null}
    </div>
