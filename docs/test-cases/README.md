...
errors are extracted from source, so the catalogue cannot drift from what runs). **422
...
| **I7** | Σ(all vaults) == trader in − trader out + LP in − LP out + insurance in − liquidator rewards − treasury sweeps |
